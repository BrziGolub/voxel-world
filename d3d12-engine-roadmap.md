...
- [x] Assert macro that logs to file
...
| 2026-07-14 | 1 | Logger file sink — log.cpp ONLY; log.h and assert.h untouched, so VE_ASSERT and every VE_LOG_* got file logging for free. That's the payoff of the single-choke-point design: every level funnels through `LogMessage`, so a new sink = a new block in one function. Future sinks (overlay, network) work the same way; when sink count grows, refactor *inside* log.cpp to a Sink list — still invisible to callers. Shape: format ONCE (`fmt::format` prefix + `fmt::vformat` message into one `std::string`), then emit to each sink — sinks can never disagree. File handle: lazy-static `FileState` (time.cpp pattern — first use IS init, nothing to forget to call) + RAII destructor (Window pattern) with null-guarded `fclose` — `fclose(nullptr)` is UB, and the guard must match the one at the use site (if a pointer needs a null check in one place, ask where else it flows). Open-failure reporting: console print DIRECTLY in the constructor, NOT via LogMessage — LogMessage→LogFile()→FileState ctor→LogMessage would re-enter the static's own initialization = UB/deadlock. Lesson: infrastructure can't depend on itself while it's being built (loggers that allocate, allocators that log — same trap, Phase 7). WAR STORY 1: `fmt::print(log)` doesn't compile — fmt format strings are compile-time-checked (consteval parse), and a runtime string can't be. fmt is RIGHT: treating runtime data as a format string is the classic `printf(buffer)` format-string vulnerability (a logged `{` would explode). RULE: data goes in ARGUMENTS — `fmt::print("{}", log)`. WAR STORY 2: `fopen` → C4996 deprecation → /WX error. First fix `fopen_s`, then reverted: fopen_s is optional Annex K (MSVC-only, kills portability); portable fix = plain `fopen` + `_CRT_SECURE_NO_WARNINGS` as PRIVATE define on ve_core (MSVC-only opinion, suppressed at the build system, not in source — a `#define` before includes is order-fragile). PRIVATE vs PUBLIC crystallized: VE_ASSERTS_ENABLED must be seen identically by every TU that includes assert.h (macro expands per include site); _CRT_SECURE_NO_WARNINGS only affects compiling core's own .cpp files. Ask "who must see it?". Flush policy: per-line `fflush` — crash A/B test proved it: without flush, the planted assert's "last words" died in the CRT buffer with the process; with flush they're in the file. fflush defends against process death (buffer→OS survives), not power loss (OS cache→disk needs fsync; no game logger bothers). Menu for later: severity-based flush (flush WARN+, buffer INFO) once volume grows; `setvbuf(_IONBF)` = per-line configured once. Truncate-per-run ("w") chosen: one session per file matches debugging one crash; append wins when history matters (intermittent bugs, logs mailed by other people). Also: VS Error List sorts by severity, NOT source order — topmost-error ritual applies to raw compiler output. Gitignored `*.log` (generated artifacts don't get committed). Known limitation: LogMessage needs a mutex when Phase 7 adds job threads (one choke point = one lock). PHASE 1 DONE except deliberate deferrals: mouse→Phase 4, assert dialog + CI vcpkg caching optional. |
